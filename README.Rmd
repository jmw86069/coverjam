...
The goal of coverjam is to provide flexible methods to create
genome sequence coverage heatmaps.
These heatmaps typically represent sequence coverage (depth):
...
from the transcript start site (TSS) to transcript termination site (TTS).
...
* optionally split rows by aforementioned clustering, and/or by
...
genome region type, additional metrics such as RNA-seq fold changes, etc.
...
using the R environment of the installing user, setting the appropriate
R_LIBS and RHOME variables to ensure the correct R is used.
...
The methods are being migrated out of the `platjam` package,
and into this package for more focused maintenance.
